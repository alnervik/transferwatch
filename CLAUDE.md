...
3. **Frontend** (`index.html`) — Vanilla JS dashboard that queries Supabase directly via its REST API, displays trade routes grouped by world, calculates profit after transfer cost (750 Tibia Coins), and lets users trigger targeted scans via the GitHub Actions API. `item_offers` is fetched with Range-paginated requests (`fetchAllRows`, 1000 rows/page — PostgREST caps single responses at its Max Rows setting) and rows older than `OFFERS_MAX_AGE_MS` (36h) are dropped at load so stale boards can't render as ✓-verified.
